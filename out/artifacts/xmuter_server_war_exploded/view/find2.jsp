...
<div id="container">
    ...
    <main>
        <div class="cbox">
            <form action="${base_path}find3.jsp" method="get">
                <table>
                    <tr>
                    ...
                    <tr>
                        <td class="ctit">密保问题</td>
                        <td colspan="2">
                            <input type="text" readonly name="secure_problem"
                                   value="${sessionScope.find_res.secure_problem}">
                        </td>
                    </tr>
                    <tr>
                        <td class="ctit">密保答案</td>
                        <td colspan="1">
                            <input class="ci" type="text" name="secure_answer">
                            <%--                            <button class="cbtn"><a>获取验证码</a></button>--%>
                        </td>
                        <input type="hidden" name="uno" value="${sessionScope.find_res.uno}">
                    </tr>
                    <tr>
                        <td colspan="3">
                            <button class="cbtn csub" onclick="check_answer()">下一步</button>
                        </td>
                    </tr>
                </table>
            </form>
...
<script>
    function check_answer() {
        ...
        var answer = form.elements["secure_answer"].value;

        if (answer == "${sessionScope.find_res.secure_answer}") {
            window.location.href = "${base_path}find3.jsp";
        } else {
            alert("answer was wrong");
            ${pageContext.request.session.invalidate()}//clear the session
            <%--window.location.href = "${base_path}find.jsp";--%> //不能用
            window.history.go(-1);
        }
    }
